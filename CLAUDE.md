...
| `src/dedup.ts` | Cross-day dedup cache (SQLite, `digests/dedup.db`): seen repos/issues/PRs/releases/HF models/ArXiv papers/community posts |
...
- Cross-day dedup (`digests/dedup.db`, SQLite) is committed to git on every run. It tracks seen trending repos (`TRENDING_DEDUP_DAYS`, default 7), seen issues/PRs/releases (`ITEM_DEDUP_DAYS`, default 30), seen HF trending models (`HF_DEDUP_DAYS`, default 7, keyed by model `id`), seen ArXiv papers (`ARXIV_DEDUP_DAYS`, default 7, keyed by paper `id`), and seen Dev.to/Lobsters posts (`COMMUNITY_DEDUP_DAYS`, default 7, keyed by article `id` / story `url`) so they aren't reported twice.
